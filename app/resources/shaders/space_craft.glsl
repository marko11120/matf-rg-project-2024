...
void main(){
   ...
   //POINT
    ...
    vec3 ambient = 0.4f * light.intensity * light.ambient * texture(texture_diffuse1, TexCoords).rgb;
    ambient *= attenuation;

    vec3 lightDir = normalize(light.position - FragPos);
    float diff = max(dot(normalize(Normal), lightDir), 0.0);
    vec3 diffuse = light.intensity * light.diffuse * texture(texture_diffuse1, TexCoords).rgb * diff;
    diffuse *= attenuation;
    ...
    vec3 specular = light.intensity * light.specular * texture(texture_specular1, TexCoords).rgb * spec * specularStrength;
    ...
    vec3 diffuse_spotLight = spotLight.diffuse * texture(texture_diffuse1, TexCoords).rgb * diff;

    //specular
    viewDir = normalize(cameraPos - FragPos);
    vec3 reflectDir = normalize(reflect(lightDir, Normal));
    spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
    vec3 specular_spotLight = spotLight.specular * texture(texture_specular1, TexCoords).rgb * spec * specularStrength;
